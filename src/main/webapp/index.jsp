...
<body>
	<div id="container">
		<header>
			<h2>머리글부분</h2>
			<c:url var="login" value="/member/login.jsp" context="${pageContext.servletContext.contextPath }"/>
			<c:if test="${empty cookie.loginId }">
			<form action="${login }" style="text-align: right;" method="post">
				<label>아이디 : <input type="text" name="id" value="${cookie.saveId.value }"></label> 
			...
			<c:if test="${not empty cookie.loginId }">
				<p style="color: blue; text-align: right;"><strong>${cookie.loginId.value }</strong>님이 로그인 중 
				<a href="/jsp/member/logout.jsp">로그아웃</a></p>
			</c:if>
		</header>
		<nav>
			<ul>
